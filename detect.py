...
colorDict = {'red': "r", 'green': "g", 'blue': "b", 'yellow': "y"}
# 创建捕获视频的实例
...
cv2.imwrite("output/origin.jpg", frame)
...
# 清除输出文件内容
with open(r'output/output.txt', 'a+', encoding='utf-8') as content1:
    content1.truncate(0)
with open(r'output/displayOutput.txt', 'a+', encoding='utf-8') as content2:
    content2.truncate(0)
...
for contour in contours:
    if cv2.contourArea(contour) > 2000:  # 根据实际情况调整面积阈值
        ...
        # 输出颜色和坐标
        if predict:
            with open(r'output/displayOutput.txt', 'a', encoding='utf-8') as f1:
                print(f"Detected {predict} cube at ({center_x}, {center_y})", file=f1)
            with open(r'output/output.txt', 'a', encoding='utf-8') as f2:
                print(colorDict[predict]+' '+str(center_x)+' '+str(center_y), file=f2)
            # 绘制边界框
